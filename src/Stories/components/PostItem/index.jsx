...
const PostItem = (props) => {
  ...
  return (
    <div className="my-post-item">
      <div className="my-post-item__user-box">
        <div className="my-post-item__user-avatar-box">
          <Link to={`/user/${props.user}`}><img src={props.avatar} alt={props.user}/></Link>
        </div>
        <div className="my-post-item__user-info">
          <div className="my-post-item__username">
            <Link to={`/user/${props.user}`}>{props.username}</Link>
          </div>
          <div className="my-post-item__date">{props.date}</div>
        </div>
      </div>
      <Link to={`/story/${props.id}`}>
        <div className="my-post-item__title">{props.title}</div>
      </Link>
      <div className="my-post-item__text">{props.text}</div>
      <div className="my-post-item__community-box">
        <div className="my-post-item__likes-box">
          <div
            className="my-post-item__like-icon"
            onClick={() => props.likePost(props.id, props.liked)}
          >
            <svg width="25" height="25" viewBox="0 0 25 25">
              {props.liked ? likedIcon() : notLikedIcon()}
            </svg>
          </div>
          <div className="my-post-item__like-count">{props.likesCount}</div>
        </div>
      </div>
    </div>
  )
